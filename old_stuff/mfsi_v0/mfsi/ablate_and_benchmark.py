...
def benchmark_jitted(fun, arg, repeats=30):
...
def run_benchmarks(p0):
    ...
    for n in sizes:
        ...
        for mode in ("unrolled", "implicit", "stop"):
            ...
            row[f"forward_{mode}"] = benchmark_jitted(fwd, p0, repeats=20)
            row[f"value_grad_{mode}"] = benchmark_jitted(vg, p0, repeats=20)
        scaling.append(row)
    ...
    t1_b = benchmark_jitted(t1, p0, repeats=50)
    t2_b = benchmark_jitted(t2, ref, repeats=50)
    fused_b = benchmark_jitted(fused, p0, repeats=50)
...
def main():
    ...
    print(json.dumps({
        ...
        "component_breakdown": benchmark["component_breakdown_n1001_t0_5"],
    }, indent=2))
