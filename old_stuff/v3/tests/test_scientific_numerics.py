...
def test_dual_calibration_uses_larger_independent_final_ensemble() -> None:
    support = build_population_support(8)
    target = distribution_summaries(
        conditioned_probabilities(params(), support, 0.9), support
    )["pair_mean"]
    result = calibrate_dual(
        params(),
        support,
    ...
    assert result.final_ensemble.indices.size == 1800
    assert all(entry["effective_sample_size"] <= 300.0 + 1e-9 for entry in result.fit_trace)
