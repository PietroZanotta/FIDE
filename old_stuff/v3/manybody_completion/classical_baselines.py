...
def one_shot_reweight_probabilities(
    ...
    *,
    ...
    name: str = "One-Shot-Reweight",
) -> tuple[BaselineDistribution, np.ndarray, np.ndarray]:
    rng = np.random.default_rng(seed)
    prior = np.asarray(prior_probabilities_array, dtype=np.float64)
    ...
    return (
        BaselineDistribution(atom_probabilities, dual, ess, name),
        indices,
        weights,
    )
...
def one_shot_reweight(
    ...
    *,
    ...
    return one_shot_reweight_probabilities(
