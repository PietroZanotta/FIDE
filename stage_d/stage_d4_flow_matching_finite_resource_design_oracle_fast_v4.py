...
def build_learned_beta_constraints(model, evaluator, d2, d3, c2, eta: np.ndarray, margin: float):
    ...
    for kt, t in enumerate(times):
        ...
        # The endpoint moments are anchored exactly.  A strict interior margin
        # must NOT be imposed at t=0 or t=1 because beta has no effect there
        # (z=t(1-t)=0), and the physical endpoint can legitimately lie on a
        # moment-hull facet.  For interior times use a tapered moment-space
        # margin that vanishes continuously at the endpoints and reaches the
        # requested value at t=1/2.
        is_endpoint = abs(z) < 1.0e-14
        time_margin = 0.0 if is_endpoint else float(margin) * (4.0 * z)
...
def evaluate_particle_law_curve_fast(
    ...
    lam_vec = np.zeros((len(times), 2), dtype=np.float64)
    cal_iterations = np.zeros(len(times), dtype=np.int64)
    ...
    for kt, _t in enumerate(times):
        ...
        lam_vec[kt] = np.asarray(lam, dtype=np.float64)
        cal_iterations[kt] = int(diag.get("iterations", -1))
    k_cal = int(np.argmax(cal))
    return {
        "heldout_mmd2": d3_trap_average(law, tw, heldout_mask),
        "min_ess_fraction": float(np.min(ess)),
        "max_calibration_residual": float(cal[k_cal]),
        "max_calibration_time_index": k_cal,
        "max_calibration_time": float(times[k_cal]),
        "lambda_norm_at_max_calibration": float(lam_norm[k_cal]),
        "lambda_vector_at_max_calibration": lam_vec[k_cal].tolist(),
        "max_abs_lambda_component_at_max_calibration": float(np.max(np.abs(lam_vec[k_cal]))),
        "calibration_iterations_at_max_calibration": int(cal_iterations[k_cal]),
        "ess_at_max_calibration": float(ess[k_cal]),
...
def evaluate_learned_trial_law_fast(
    ...
    return {
        ...
        "finite_max_calibration_time_index": int(finite["max_calibration_time_index"]),
        "finite_max_calibration_time": float(finite["max_calibration_time"]),
        "finite_lambda_norm_at_max_calibration": float(finite["lambda_norm_at_max_calibration"]),
        "finite_lambda_vector_at_max_calibration": list(finite["lambda_vector_at_max_calibration"]),
        "finite_max_abs_lambda_component_at_max_calibration": float(finite["max_abs_lambda_component_at_max_calibration"]),
        "finite_calibration_iterations_at_max_calibration": int(finite["calibration_iterations_at_max_calibration"]),
        "finite_ess_at_max_calibration": float(finite["ess_at_max_calibration"]),
        "finite_max_lambda_norm": float(finite["max_lambda_norm"]),
...
def build_arg_parser():
    ...
    p.add_argument(
        "--feasibility-margin", type=float, default=None,
        help="Peak interior moment-hull margin. In v3 it is tapered as "
             "4*t*(1-t)*margin and is exactly zero at t=0,1 so endpoint "
             "anchoring is not made artificially infeasible."
    )
    ...
    p.add_argument(
        "--calibration-steps", type=int, default=None,
        help="Override the empirical tilt Newton iteration budget (reference default: 28)."
    )
    p.add_argument(
        "--lambda-clip", type=float, default=None,
        help="Override the per-coordinate empirical tilt multiplier clip (reference default: 80)."
    )
    p.add_argument(
        "--newton-step-cap", type=float, default=None,
        help="Override the empirical tilt Newton step-norm cap (reference default: 5)."
    )
    p.add_argument(
    ...
    p.add_argument(
        "--stop-after-stage2", action="store_true",
        help="Run through the finite-resource law screen, report/save Stage-2 survivors, "
             "and exit before any weighted-Poisson action solves."
    )
    return p
...
def main():
    ...
    for key in (
        ...
        "calibration_steps", "lambda_clip", "newton_step_cap",
    ):
    ...
    print(f"Tilt solver   : steps={cfg.calibration_steps}, lambda_clip={cfg.lambda_clip:.1f} per coordinate, "
          f"Newton step cap={cfg.newton_step_cap:.1f}")
            ...
            for trial_i, shared in enumerate(law_bank):
                ...
                if row["finite_max_calibration_resid"] > cfg.max_finite_calibration_resid:
                    gate_failure = (
                        ...
                        f"t={row['finite_max_calibration_time']:.3f}, "
                        f"|lambda|={row['finite_lambda_norm_at_max_calibration']:.3e}, "
                        f"lambda={np.round(row['finite_lambda_vector_at_max_calibration'], 4).tolist()}, "
                        f"max|lambda_j|={row['finite_max_abs_lambda_component_at_max_calibration']:.3e}, "
                        f"iters={row['finite_calibration_iterations_at_max_calibration']}, "
                        f"ESSrel={row['finite_ess_at_max_calibration']:.3e}, "
                        f"projection_active={bool(row['feasibility_projection_active'])}, "
                        f"projection_norm={row['feasibility_projection_norm']:.3e}, "
                        f"unconstrained_hull_violation={row['max_unconstrained_hull_violation']:.3e}"
                    )
                    break
    ...
    if args.stop_after_stage2:
        preflight_path = Path(str(args.output) + ".stage2.json")
        preflight_payload = {
            "stage": "D.4 Stage-2 preflight",
            ...
            "population_feasible": [public_candidate(c) for c in pop_feasible],
            "finite_numerically_valid": [public_candidate(c) for c in finite_valid],
            "finite_risk_feasible": [public_candidate(c) for c in finite_feasible],
        }
        preflight_path.write_text(
            json.dumps(jsonify(preflight_payload), indent=2, allow_nan=True) + "\n",
            encoding="utf-8",
        )
        print("\nStage-2 preflight complete; stopping before Poisson/action work.", flush=True)
        print(f"  numerically valid Stage-2 candidates: {len(finite_valid)}", flush=True)
        print(f"  finite-risk feasible candidates:      {len(finite_feasible)}", flush=True)
        for c in finite_feasible:
            print(
                f"  survivor eta={np.round(c['theta_deg'], 5).tolist()} | "
                f"R={c['finite_risk']['mean']:.8e} +/- {c['finite_risk']['se']:.2e} | "
                f"ESSmin={c['finite_min_ess']:.3f} | calmax={c['finite_max_cal']:.3e}",
                flush=True,
            )
        print(f"  saved Stage-2 preflight: {preflight_path.resolve()}", flush=True)
        return
    ...
    payload = {
        ...
        "interpretation_notes": [
            ...
            "The finite feasibility margin is applied only at interior times, tapered by 4*t*(1-t), and is zero at the endpoint anchors.",
