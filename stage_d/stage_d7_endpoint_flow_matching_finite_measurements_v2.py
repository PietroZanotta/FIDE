...
class D7Config:
    ...
    # Solver target versus scientific validity gates.
    # The solver still tries to reach solver_accept_tol, but finite/noisy rows
    # use the deliberately looser Stage-D4-style validity threshold because
    # reconstructed targets can sit close to the empirical moment-hull boundary.
...
def robust_empirical_tilt(
    ...
    for retry in range(int(cfg.calibration_max_retries) + 1):
        ...
        # D2/D6 relative ESS convention for nonuniform Gauss-Hermite weights:
        #
        #     ESS_rel = ESS(projected) / ESS(base)
        #             = [1/sum(w^2)] / [1/sum(a^2)].
        #
        # This is intentionally NOT the chi-square importance-ratio ESS
        # 1/sum(w^2/a), which can be orders of magnitude smaller for GH weights.
        bw = base_w / np.sum(base_w)
        ess_projected = 1.0 / max(float(np.sum(w * w)), 1.0e-300)
        ess_base = 1.0 / max(float(np.sum(bw * bw)), 1.0e-300)
        ess_rel = float(ess_projected / max(ess_base, 1.0e-300))

        # Keep the importance-ratio quantity as a diagnostic only.
        ratio = w / np.maximum(bw, 1.0e-300)
        chi2_ess_fraction = float(1.0 / max(np.sum(bw * ratio * ratio), 1.0e-300))
        ...
        rec = {
            ...
            "chi2_importance_ess_fraction": chi2_ess_fraction,
        }
        attempts.append(rec)
        ...
        if resid <= cfg.solver_accept_tol:
            break
        start = lam
...
def evaluate_particle_curve(
    ...
    compute_action: bool,
    calibration_validity_tol: float,
):
    ...
    chi2_ess = np.zeros(len(times), dtype=np.float64)
    ...
    for kt, t in enumerate(times):
        ...
        chi2_ess[kt] = float(dd.get("chi2_importance_ess_fraction", np.nan))
    ...
    valid = bool(
        np.all(hull_inside)
        and np.max(cal) <= float(calibration_validity_tol)
    ...
    return {
        ...
        "min_chi2_importance_ess_fraction": float(np.nanmin(chi2_ess)),
        "mean_chi2_importance_ess_fraction": float(np.nansum(tw * chi2_ess)),
        "max_calibration_residual": float(np.max(cal)),
        "calibration_validity_tol": float(calibration_validity_tol),
...
def evaluate_design_trial(
    ...
    finite = evaluate_particle_curve(
        ...
        calibration_validity_tol=float(cfg.max_finite_calibration_resid),
    )
    ...
    row = {
        ...
        "finite_min_chi2_ess": finite["min_chi2_importance_ess_fraction"],
        ...
        "exact_min_chi2_ess": exact["min_chi2_importance_ess_fraction"],
...
def build_arg_parser():
    ...
    p.add_argument("--output", type=str, default="stage_d7_endpoint_flow_matching_finite_measurements_v2.json")
...
def main():
    ...
    print(
        f"Validity      : population cal <= {cfg.max_population_calibration_resid:.1e}, "
    ...
    checks = {
        ...
        "ess_gate_metric": "ESS(projected)/ESS(base)",
    ...
    payload = {
        ...
        "interpretation": [
            ...
            "Relative ESS uses the exact D.2/D.6 convention ESS(projected)/ESS(base), which is the appropriate comparison for nonuniform Gauss-Hermite quadrature weights; a chi-square importance-ratio ESS is reported only as an auxiliary diagnostic.",
            "Population and finite/noisy calibration validity use separate thresholds, matching the Stage-D4 convention that noisy reconstructed targets are allowed a looser numerical calibration gate.",
