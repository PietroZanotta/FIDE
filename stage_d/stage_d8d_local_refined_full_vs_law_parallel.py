...
def parser():
    ...
    p.add_argument("--workers", type=int, default=min(6, max(1, os.cpu_count() or 1)),
                   help="Thread workers for population/finite-law CPU-heavy candidate evaluation. Shared D5/JAX evaluator; no duplicate GPU contexts.")
    p.add_argument("--action-workers", type=int, default=2,
                   help="Thread workers for Poisson/full-action phases. Keep small on a single GPU (usually 1-2).")
    p.add_argument("--blas-threads", type=int, default=1,
                   help="Inner BLAS/OpenMP threads per process while outer candidate threads run; 1 avoids oversubscription.")
...
def main():
    ...
    if args.workers < 1 or args.action_workers < 1 or args.blas_threads < 1:
        raise ValueError("workers, action-workers, and blas-threads must all be >=1")
    ...
    print(f"Parallelism   : workers={args.workers}, action_workers={args.action_workers}, BLAS threads={args.blas_threads}")
    print(f"JAX backend   : {jax.default_backend()} | devices={[str(d) for d in jax.devices()]}")
    ...
    def _population_job(c):
        return d8.population_endpoint_fm_law(model, evaluator, d2, d7, np.asarray(c["eta"]), cfg)
    ...
    for c in candidates:
        ...
        if k in progress["population"]:
            ...
            missing.append(c)
    if done:
        print(f"  reused population cache: {done}/{len(candidates)}", flush=True)
    if missing:
        with threadpool_limits(limits=int(args.blas_threads)):
            with ThreadPoolExecutor(max_workers=int(args.workers), thread_name_prefix="d8d-pop") as pool:
                futs = {pool.submit(_population_job, c): c for c in missing}
                for fut in as_completed(futs):
                    c = futs[fut]
                    k = eta_key(c["eta"])
                    c["population"] = fut.result()
                    progress["population"][k] = jsonify(c["population"])
                    write_json(progress_path, progress)
                    done += 1
                    if done % max(1, len(candidates)//10) == 0 or done == len(candidates):
                        print(f"  population {done}/{len(candidates)}", flush=True)
    ...
    def _constraint_job(c):
        try:
            cc = d7.build_joint_beta_constraints(
    ...
    with threadpool_limits(limits=int(args.blas_threads)):
        with ThreadPoolExecutor(max_workers=int(args.workers), thread_name_prefix="d8d-hull") as pool:
            futs = {pool.submit(_constraint_job, c): c for c in pop_feasible}
            for fut in as_completed(futs):
                c = futs[fut]
                ok, cc, err = fut.result()
                if ok:
                    c["constraints"] = cc
                    feasible_for_finite.append(c)
                else:
                    c["constraint_error"] = err
                completed_constraints += 1
                if completed_constraints % max(1, len(pop_feasible)//10) == 0 or completed_constraints == len(pop_feasible):
                    print(f"    constraints {completed_constraints}/{len(pop_feasible)}", flush=True)
    # Restore deterministic candidate order after as_completed().
    finite_keys = {eta_key(c["eta"]): c for c in feasible_for_finite}
    feasible_for_finite = [c for c in pop_feasible if eta_key(c["eta"]) in finite_keys]
    ...
    for c in feasible_for_finite:
        ...
        if cached is not None:
            ...
            print(
                f"  law cached {done}/{len(feasible_for_finite)} eta={np.degrees(c['eta']).round(3).tolist()} | "
                f"R={cached['finite_risk']['mean']:.8e} +/- {cached['finite_risk']['se']:.2e} | valid={cached['valid']}",
                flush=True,
            )
        else:
            missing.append(c)
    ...
    if missing:
        print(f"  parallel finite-law jobs: {len(missing)} candidates x {len(law_bank)} trials on {args.workers} workers", flush=True)
        with threadpool_limits(limits=int(args.blas_threads)):
            with ThreadPoolExecutor(max_workers=int(args.workers), thread_name_prefix="d8d-law") as pool:
                futs = {pool.submit(_finite_law_job, c): c for c in missing}
                for fut in as_completed(futs):
                    c = futs[fut]
                    rec = fut.result()
                    k = eta_key(c["eta"])
                    ...
                    print(
                        f"  law {done}/{len(feasible_for_finite)} eta={np.degrees(c['eta']).round(3).tolist()} | "
                        f"R={rec['finite_risk']['mean']:.8e} +/- {rec['finite_risk']['se']:.2e} | valid={rec['valid']}",
                        flush=True,
                    )
    ...
    for c in law_feasible:
        k = eta_key(c["eta"])
        cached = progress["pre_action"].get(k)
        if cached is not None and len(cached.get("rows", [])) >= int(args.pre_action_trials):
            c["pre_action"] = cached
            done += 1
            print(f"  pre cached {done}/{len(law_feasible)} eta={np.degrees(c['eta']).round(3).tolist()} | A={cached['action']['mean']:.6e} | valid={cached['valid']}", flush=True)
        else:
            missing.append(c)
    if missing:
        with threadpool_limits(limits=int(args.blas_threads)):
            with ThreadPoolExecutor(max_workers=int(args.action_workers), thread_name_prefix="d8d-preA") as pool:
                futs = {pool.submit(_pre_action_job, c): c for c in missing}
                for fut in as_completed(futs):
                    c = futs[fut]
                    rec = fut.result()
                    k = eta_key(c["eta"])
                    ...
                    print(f"  pre {done}/{len(law_feasible)} eta={np.degrees(c['eta']).round(3).tolist()} | A={rec['action']['mean']:.6e} | valid={rec['valid']}", flush=True)
    ...
    for c in shortlist:
        ...
        if cached is not None and len(cached.get("rows", [])) >= int(args.action_trials):
            c["refined_action"] = cached
            done += 1
            print(f"  refined cached {done}/{len(shortlist)} eta={np.degrees(c['eta']).round(3).tolist()} | A={cached['action']['mean']:.6e} +/- {cached['action']['se']:.2e} | valid={cached['valid']}", flush=True)
        else:
            missing.append(c)
    if missing:
        with threadpool_limits(limits=int(args.blas_threads)):
            with ThreadPoolExecutor(max_workers=int(args.action_workers), thread_name_prefix="d8d-refA") as pool:
                futs = {pool.submit(_refined_action_job, c): c for c in missing}
                for fut in as_completed(futs):
                    c = futs[fut]
                    rec = fut.result()
                    k = eta_key(c["eta"])
                    ...
                    print(f"  refined {done}/{len(shortlist)} eta={np.degrees(c['eta']).round(3).tolist()} | A={rec['action']['mean']:.6e} +/- {rec['action']['se']:.2e} | valid={rec['valid']}", flush=True)
    ...
    # Two selected designs can run concurrently; each reuses the single loaded D5 bank.
    with threadpool_limits(limits=int(args.blas_threads)):
        with ThreadPoolExecutor(max_workers=min(int(args.action_workers), len(selected)), thread_name_prefix="d8d-val") as pool:
            futs = {pool.submit(_validation_job, name, c): (name, c) for name, c in selected.items()}
            for fut in as_completed(futs):
                name, c = futs[fut]
                _, rows = fut.result()
                k = eta_key(c["eta"])
