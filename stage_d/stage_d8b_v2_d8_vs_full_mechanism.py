#!/usr/bin/env python3
"""
Stage D.8b-v2 diagnostic: WHY does the endpoint-trained D.8 sensor behave differently
...
def true_raw_reference_law(model, evaluator, d2):
    """MMD of the unprojected D5 particle reference against the external law.

    This quantity is independent of sensor design eta.  It is computed directly
    from base quadrature weights, rather than relying on any D7 field named
    "raw_heldout_mmd2", whose semantics may differ.
    """
    node_name, nodes = find_array_attr(
        evaluator,
        ("learned_nodes", "nodes", "xnodes", "x_nodes", "reference_nodes", "particle_nodes"),
    ...
    if nodes is None or base_w is None:
        return {"available": False, "reason": "Could not locate learned nodes/base weights."}
    ...
    bw = float(getattr(evaluator.cfg, "kde_bandwidth", 0.0))
    if bw <= 0.0:
        bw = 0.35 * float(model.dx)
    truncate = float(getattr(evaluator.cfg, "kde_truncate", 4.0))
    ...
    for kt, t in enumerate(times):
        ...
        w = np.asarray(base_w[mask], dtype=np.float64)
        w /= max(float(np.sum(w)), 1e-300)
        _, qmass, _, _ = d2.rasterize_projected_state(
            model=model,
            x=x,
            w=w,
        ...
        for ka, alpha in enumerate(alphas):
            _, pmass = model.external_q_mass(jnp.asarray(t), jnp.asarray(alpha))
            mmd = float(model.gaussian_mmd2_mass(
                jnp.asarray(qmass, dtype=jnp.float64),
                jnp.asarray(pmass, dtype=jnp.float64),
            ))
            lt += aw[ka] * mmd
        per_time.append({"t": float(t), "raw_reference_mmd2": float(lt)})
        total += tw[kt] * lt
    return {
        "available": True,
        "raw_reference_mmd2": float(total),
        "per_time": per_time,
        "node_attribute": node_name,
        "base_weight_attribute": base_name,
    }


def projected_forcing_profiles(model, evaluator, d2, d8, designs, solver_cfg):
    """Exact-population projected forcing/action decomposition.

    Uses the same D5 particles, exact scientific moment curves, D2 particle forcing
    calculus and weighted-grid Poisson oracle.  This isolates the downstream reason
    that a design with a larger tilt can nevertheless have a lower Poisson action.
    """
    node_name, nodes = find_array_attr(
        evaluator,
        ("learned_nodes", "nodes", "xnodes", "x_nodes", "reference_nodes", "particle_nodes"),
    ...
    if nodes is None or unodes is None or base_w is None:
        return {
            "available": False,
            "reason": "Could not locate D5 learned nodes, velocities, and base weights.",
        }
    ...
    tw = tw / np.sum(tw)
    aw = aw / np.sum(aw)
    bw = float(getattr(solver_cfg, "kde_bandwidth", 0.0))
    if bw <= 0.0:
        bw = 0.35 * float(model.dx)
    truncate = float(getattr(solver_cfg, "kde_truncate", 4.0))
    ridge = float(model.cfg.newton_ridge)

    out = {
        "available": True,
        "discovered_attributes": {
            "nodes": node_name, "velocities": vel_name, "base_weights": base_name,
        },
        "designs": {},
    }

    for name, eta in designs.items():
        curves = [d8.exact_target_curve(model, evaluator, eta, float(a)) for a in alphas]
        lam_warm = [np.zeros(2, dtype=np.float64) for _ in alphas]
        per_time = []
        acc = {
            "projected_law_mmd2": 0.0,
            "full_poisson_action": 0.0,
            "particle_forcing_power": 0.0,
            "grid_forcing_power": 0.0,
            "time_term_power": 0.0,
            "advection_term_power": 0.0,
            "time_advection_cross": 0.0,
            "tangent_action": 0.0,
            "hidden_action": 0.0,
            "poisson_amplification_weighted": 0.0,
            "lambda_norm": 0.0,
            "lambda_dot_norm": 0.0,
            "ess_fraction": 0.0,
            "calibration_residual": 0.0,
            "projected_cov_min_eig": 0.0,
            "projected_cov_condition": 0.0,
        }
        min_ess = float("inf")
        max_lam = 0.0
        max_lamdot = 0.0
        ...
        for kt, t in enumerate(times):
            ...
            x = x_all[mask]
            u = u_all[mask]
            w0 = np.asarray(base_w[mask], dtype=np.float64)
            w0 /= max(float(np.sum(w0)), 1e-300)
            phi, grad_phi = d2.sensor_particle_fields(model, eta, x)

            tv = {k: 0.0 for k in (
                "projected_law_mmd2", "full_poisson_action",
                "particle_forcing_power", "grid_forcing_power",
                "time_term_power", "advection_term_power", "time_advection_cross",
                "tangent_action", "hidden_action", "poisson_amplification",
                "lambda_norm", "lambda_dot_norm", "ess_fraction",
                "calibration_residual", "projected_cov_min_eig",
                "projected_cov_condition",
            )}

            for ka, alpha in enumerate(alphas):
                target = np.asarray(curves[ka][0][kt], dtype=np.float64)
                cdot = np.asarray(curves[ka][1][kt], dtype=np.float64)
                ...
                lam_warm[ka] = np.asarray(st["lambda"], dtype=np.float64)

                w = np.asarray(st["weights"], dtype=np.float64)
                h = np.asarray(st["h"], dtype=np.float64)
                lam = np.asarray(st["lambda"], dtype=np.float64)
                lamdot = np.asarray(st["lambda_dot"], dtype=np.float64)
                moment = np.asarray(st["moment"], dtype=np.float64)
                C = np.asarray(st["C"], dtype=np.float64)
                Csym = 0.5 * (C + C.T)
                cev = np.linalg.eigvalsh(Csym)
                cmin = float(np.min(cev))
                ccond = float(np.max(cev) / max(cmin, 1e-14))

                m = np.einsum("nmc,nc->nm", grad_phi, u)
                g = m @ lam
                Eg = float(np.dot(w, g))
                time_term = (phi - moment[None, :]) @ lamdot
                adv_term = g - Eg
                # st["h"] has the final centering safeguard.  Its power is the
                # actual particle forcing diagnostic.
                p_h = float(np.dot(w, h * h))
                p_time = float(np.dot(w, time_term * time_term))
                p_adv = float(np.dot(w, adv_term * adv_term))
                p_cross = float(2.0 * np.dot(w, time_term * adv_term))

                q, qmass, h_grid, _ = d2.rasterize_projected_state(
                    model=model, x=x, w=w, h=h,
                    bandwidth=bw, truncate=truncate,
                )
                ...
                full = float(full)
                grid_power = float(np.sum(qmass * h_grid * h_grid))
                amp = full / max(grid_power, 1e-300)

                _, pmass = model.external_q_mass(jnp.asarray(t), jnp.asarray(alpha))
                law = float(model.gaussian_mmd2_mass(
                    jnp.asarray(qmass, dtype=jnp.float64),
                    jnp.asarray(pmass, dtype=jnp.float64),
                ))
                tangent = float(st["diagnostics"].get("tangent_action", np.nan))
                hidden = full - tangent if np.isfinite(tangent) else float("nan")
                ess = float(st["diagnostics"].get("ess_fraction", np.nan))
                cal = float(st["diagnostics"].get("residual", np.nan))

                vals = {
                    "projected_law_mmd2": law,
                    "full_poisson_action": full,
                    "particle_forcing_power": p_h,
                    "grid_forcing_power": grid_power,
                    "time_term_power": p_time,
                    "advection_term_power": p_adv,
                    "time_advection_cross": p_cross,
                    "tangent_action": tangent,
                    "hidden_action": hidden,
                    "poisson_amplification": amp,
                    "lambda_norm": float(np.linalg.norm(lam)),
                    "lambda_dot_norm": float(np.linalg.norm(lamdot)),
                    "ess_fraction": ess,
                    "calibration_residual": cal,
                    "projected_cov_min_eig": cmin,
                    "projected_cov_condition": ccond,
                }
                for k, v in vals.items():
                    tv[k] += aw[ka] * float(v)

                min_ess = min(min_ess, ess)
                max_lam = max(max_lam, float(np.max(np.abs(lam))))
                max_lamdot = max(max_lamdot, float(np.max(np.abs(lamdot))))
                max_cal = max(max_cal, cal)
                min_cov = min(min_cov, cmin)
                max_cov_cond = max(max_cov_cond, ccond)

            per_time.append({"t": float(t), **{k: float(v) for k, v in tv.items()}})
            for k in (
                "projected_law_mmd2", "full_poisson_action",
                "particle_forcing_power", "grid_forcing_power",
                "time_term_power", "advection_term_power", "time_advection_cross",
                "tangent_action", "hidden_action", "lambda_norm", "lambda_dot_norm",
                "ess_fraction", "calibration_residual", "projected_cov_min_eig",
                "projected_cov_condition",
            ):
                acc[k] += tw[kt] * tv[k]
            acc["poisson_amplification_weighted"] += tw[kt] * tv["poisson_amplification"]

        # Ratio of integrated action to integrated forcing is easier to interpret
        # than the average of pointwise ratios, so report both.
        acc["action_over_grid_forcing"] = (
            acc["full_poisson_action"] / max(acc["grid_forcing_power"], 1e-300)
        )
        acc["min_ess_fraction"] = float(min_ess)
        acc["max_abs_lambda_coordinate"] = float(max_lam)
        acc["max_abs_lambda_dot_coordinate"] = float(max_lamdot)
        acc["max_calibration_residual"] = float(max_cal)
        acc["min_projected_cov_eig_over_path"] = float(min_cov)
        acc["max_projected_cov_condition_over_path"] = float(max_cov_cond)
        out["designs"][name] = {
            "eta_deg": np.degrees(eta).tolist(),
            "time_integrated": {k: float(v) for k, v in acc.items()},
            "per_time": per_time,
        }
    return out
...
def build_interpretation(pop, summaries, paired, geom, raw_reference=None, forcing=None):
    notes = []
    ...
    # Do not interpret population_endpoint_fm_law["raw_lift_mmd2"] here:
    # D7's field naming may not denote the unprojected D5 reference MMD.
    ...
    if geom.get("available"):
        ...
        if gf["zero_tilt_tangent_proxy"] > 0:
            notes.append(
                ...
                "Values <1 here, together with larger Full lambda / lower Full ESS, show that raw moment mismatch does not rank final Poisson action."
            )
    if raw_reference and raw_reference.get("available"):
        raw = float(raw_reference["raw_reference_mmd2"])
        notes.append(
            f"True unprojected D5-reference MMD is sensor-independent: {raw:.6e}. "
            f"Projected improvement raw-minus-law is D8={raw-d8p['lift_mmd2']:+.4e}, "
            f"Full={raw-fp['lift_mmd2']:+.4e}."
        )
    if forcing and forcing.get("available"):
        fd = forcing["designs"]["d8"]["time_integrated"]
        ff = forcing["designs"]["full"]["time_integrated"]
        if ff["grid_forcing_power"] > 0:
            notes.append(
                f"Actual projected grid-forcing-power ratio D8/Full={fd['grid_forcing_power']/ff['grid_forcing_power']:.3f}; "
                f"full-action ratio={fd['full_poisson_action']/ff['full_poisson_action']:.3f}."
            )
        if ff["action_over_grid_forcing"] > 0:
            notes.append(
                f"Poisson amplification A/E_q[h^2] ratio D8/Full="
                f"{fd['action_over_grid_forcing']/ff['action_over_grid_forcing']:.3f}. "
                "A large ratio difference indicates that the spatial weighted-elliptic geometry, not merely forcing magnitude, is controlling the action ordering."
            )
        if np.isfinite(ff.get("tangent_action", np.nan)) and ff["tangent_action"] != 0:
            notes.append(
                f"Projected tangent-action ratio D8/Full={fd['tangent_action']/ff['tangent_action']:.3f}; "
                f"hidden/full-continuity action ratio D8/Full={fd['hidden_action']/ff['hidden_action']:.3f}. "
                "This separates low-dimensional moment-rate burden from the full law-realization burden."
            )
    return notes
...
def main():
    ...
    print("=" * 112)
    print("Stage D.8b-v2 — D8 vs historical Full-TC mechanism diagnostic")
    print("=" * 112)
    ...
    for name, eta in designs.items():
        ...
        p = population[name]
        print(
            f"  {name:5s}: projected L={p['lift_mmd2']:.8e} | D7 raw-field={p['raw_lift_mmd2']:.8e} | "
    ...
    # ---------------------------------------------------------------
    # D. Correct unprojected-reference law + actual projected forcing.
    # ---------------------------------------------------------------
    print("\n[D] True raw-reference law and calibrated forcing/Poisson decomposition...", flush=True)
    raw_reference = true_raw_reference_law(model, evaluator, d2)
    if raw_reference.get("available"):
        print(f"  sensor-independent raw D5 MMD={raw_reference['raw_reference_mmd2']:.8e}")
    else:
        print(f"  raw reference skipped: {raw_reference.get('reason')}")

    forcing = projected_forcing_profiles(model, evaluator, d2, d8, designs, d7_cfg)
    if forcing.get("available"):
        for name in ("d8", "full"):
            f = forcing["designs"][name]["time_integrated"]
            print(
                f"  {name:5s}: A={f['full_poisson_action']:.6e} | "
                f"E[h_grid^2]={f['grid_forcing_power']:.6e} | "
                f"A/F={f['action_over_grid_forcing']:.6e} | "
                f"T={f['tangent_action']:.6e} | hidden={f['hidden_action']:.6e}"
            )
    else:
        print(f"  forcing decomposition skipped: {forcing.get('reason')}")

    # ---------------------------------------------------------------
    # E. Save machine-readable attribution package + CSVs.
    # ---------------------------------------------------------------
    interpretation = build_interpretation(population, summaries, paired, geometry, raw_reference, forcing)
    output = Path(args.output)
    trial_csv = output.with_suffix(".trial_pairs.csv")
    time_csv = output.with_suffix(".time_profiles.csv")
    forcing_csv = output.with_suffix(".forcing_profiles.csv")
    ...
    if forcing.get("available"):
        forcing_rows = []
        for name in ("d8", "full"):
            for r in forcing["designs"][name]["per_time"]:
                forcing_rows.append({"design": name, **r})
        write_csv(forcing_csv, forcing_rows)

    payload = {
        "stage": "D.8b-v2 D8-vs-Full mechanism diagnostic",
        ...
        "true_unprojected_d5_reference": raw_reference,
        "projected_forcing_poisson_decomposition": forcing,
        "interpretation": interpretation,
        "important_caveats": [
            "The D7 field named raw_heldout_mmd2 is not interpreted as the unprojected D5-reference MMD in this diagnostic; the true raw reference is recomputed directly from base particle weights and is sensor-independent.",
        ...
        "csv_outputs": {
            ...
            "forcing_profiles": str(forcing_csv) if forcing.get("available") else None,
        },
    ...
    if forcing.get("available"):
        print(f"Saved forcing CSV: {forcing_csv.resolve()}")
