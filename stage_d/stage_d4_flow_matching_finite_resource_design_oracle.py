...
def main():
    ...
    # ------------------------------------------------------------------
    print("\n[1/3] Learned-FM population-law screen...", flush=True)
    valid_population: List[Dict[str, Any]] = []
    for j, cand in enumerate(candidates):
        try:
            pop = population_learned_law(model, evaluator, d2, d3, cand["eta"])
    ...
    # ------------------------------------------------------------------
    print("\n[2/3] Learned-FM finite-resource law screen...", flush=True)
    ...
    for j, cand in enumerate(pop_feasible):
        ...
        try:
            ...
            for shared in law_bank:
                row, _ = evaluate_learned_trial(
                    model, evaluator, d2, d3, c2, measurement_cov,
                    eta, shared, acq_idx, heldout_mask, cfg, constraints, False,
                )
                law_rows.append(row)
            ...
            cand["finite_numerically_valid"] = bool(
                cand["finite_min_ess"] >= cfg.min_ess_fraction
    ...
    if not finite_valid:
        raise RuntimeError("No numerically valid population-feasible candidate under finite measurements")
    ...
    # ------------------------------------------------------------------
    # Stage 3: expensive learned-FM finite action only on survivors.
    # ------------------------------------------------------------------
    ...
    for j, cand in enumerate(finite_feasible):
        ...
        for shared in action_bank:
            row, _ = evaluate_learned_trial(
                model, evaluator, d2, d3, c2, measurement_cov,
                eta, shared, acq_idx, heldout_mask, cfg, constraints, True,
            )
