"""Differentiable candidate-specific empirical I-projection trajectories.

This additive Tesseract accepts a distinct feature tensor and target trajectory
for every candidate while sharing one reference quadrature bank.  The native
kernel parallelizes candidates and warm-starts multipliers along physical time.
"""
...
try:
    import _candidate_iprojection_native as native
except ImportError as exc:  # pragma: no cover - optional build
    raise ImportError(
        "The native candidate I-projection extension is not built; see "
        "native/candidate_iprojection_tesseract/README.md."
    ) from exc
...
def _solve(inputs: InputSchema):
    ...
    args = _args(inputs)
    result = native.solve_candidate_batch(*arrays, *args)
    with _CACHE_LOCK:
...
def vector_jacobian_product(
    ...
    result = native.vjp_candidate_batch(
        phi,
        log_base,
        targets,
        lambda_values,
    ...
    return {
        name: np.asarray(result[name], dtype=np.float64, order="C")
        for name in ("phi", "log_base_weights", "targets")
        if name in vjp_inputs
    }
...
def jacobian_vector_product(
    ...
    args = _args(inputs)
    lambda_values = _cached_lambda((phi, log_base, targets), args)
    result = native.jvp_candidate_batch(
        phi,
        log_base,
        targets,
        lambda_values,
...
def abstract_eval(abstract_inputs: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    if len(shape) != 3:
        raise ValueError("targets must have abstract shape [C,T,M]")
