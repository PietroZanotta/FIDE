...
CandidateShape validate_candidate_inputs(
    const py::array& phi, const py::array& log_base, const py::array& targets) {
    for (const auto* item : {&phi, &log_base, &targets}) {
        if (!item->dtype().is(py::dtype::of<double>())) {
            throw py::type_error("candidate I-projection arrays must have dtype float64");
        }
        if (!(item->flags() & py::array::c_style)) {
            throw py::value_error("candidate I-projection arrays must be C-contiguous");
        }
    }
    if (phi.ndim() != 4) {
        throw py::value_error("phi must have shape [C,T,N,M]");
    }
    if (log_base.ndim() != 2) {
        throw py::value_error("log_base_weights must have shape [T,N]");
    }
    if (targets.ndim() != 3) {
        throw py::value_error("targets must have shape [C,T,M]");
    }
    const CandidateShape shape{
        static_cast<int>(phi.shape(0)),
        static_cast<int>(phi.shape(1)),
        static_cast<int>(phi.shape(2)),
        static_cast<int>(phi.shape(3)),
    };
    if (shape.candidates < 1 || shape.times < 1 || shape.particles < 1
        || shape.moments < 1) {
        throw py::value_error("all candidate I-projection dimensions must be positive");
    }
    if (log_base.shape(0) != shape.times || log_base.shape(1) != shape.particles
        || targets.shape(0) != shape.candidates || targets.shape(1) != shape.times
        || targets.shape(2) != shape.moments) {
        throw py::value_error("inconsistent candidate phi/log_base_weights/targets shapes");
    }
    return shape;
}
...
void validate_candidate_lambda_array(
    const py::array& array, const CandidateShape& s, const char* name) {
    if (array.ndim() != 3 || !array.dtype().is(py::dtype::of<double>())
        || !(array.flags() & py::array::c_style)
        || array.shape(0) != s.candidates || array.shape(1) != s.times
        || array.shape(2) != s.moments) {
        throw py::value_error(
            std::string(name) + " must be contiguous float64 [C,T,M]");
    }
}

py::dict solve_candidate_batch(
    const py::array& phi_array, const py::array& log_base_array,
    const py::array& targets_array, int max_steps, double residual_tol,
    double newton_ridge, double step_cap, double lambda_clip,
    int line_search_steps, double implicit_ridge) {
    const CandidateShape s = validate_candidate_inputs(
        phi_array, log_base_array, targets_array);
    const Config cfg = validate_config(
        max_steps, residual_tol, newton_ridge, step_cap, lambda_clip,
        line_search_steps, implicit_ridge);
    const auto* phi = static_cast<const double*>(phi_array.data());
    const auto* log_base = static_cast<const double*>(log_base_array.data());
    const auto* targets = static_cast<const double*>(targets_array.data());
    py::array_t<double> lambda_out({s.candidates, s.times, s.moments});
    py::array_t<std::int32_t> iteration_out({s.candidates, s.times});
    py::array_t<double> residual_out({s.candidates, s.times});
    py::array_t<bool> converged_out({s.candidates, s.times});
    auto* lambdas = lambda_out.mutable_data();
    auto* iterations = iteration_out.mutable_data();
    auto* residuals = residual_out.mutable_data();
    auto* converged = converged_out.mutable_data();
    {
        py::gil_scoped_release release;
        // Each candidate owns a complete trajectory. Static scheduling and a
        // private multiplier preserve deterministic float64 warm starts over
        // physical time while candidates run independently in parallel.
        #pragma omp parallel for schedule(static)
        for (int c = 0; c < s.candidates; ++c) {
            std::vector<double> lambda(s.moments, 0.0);
            for (int t = 0; t < s.times; ++t) {
                const std::size_t ct
                    = static_cast<std::size_t>(c) * s.times + t;
                const std::size_t pt
                    = ct * s.particles * s.moments;
                const std::size_t wt
                    = static_cast<std::size_t>(t) * s.particles;
                const std::size_t mt = ct * s.moments;
                int count = 0;
                double norm = 0.0;
                solve_one(
                    phi + pt, log_base + wt, targets + mt,
                    s.particles, s.moments, cfg, lambda, count, norm);
                for (int j = 0; j < s.moments; ++j) {
                    lambdas[mt + j] = lambda[j];
                }
                iterations[ct] = count;
                residuals[ct] = norm;
                converged[ct] = norm <= cfg.residual_tol;
            }
        }
    }
    return py::dict(
...
py::dict vjp_candidate_batch(
    const py::array& phi_array, const py::array& log_base_array,
    const py::array& targets_array, const py::array& lambda_values_array,
    const py::array& lambda_bar_array, int max_steps, double residual_tol,
    double newton_ridge, double step_cap, double lambda_clip,
    int line_search_steps, double implicit_ridge) {
    const CandidateShape s = validate_candidate_inputs(
        phi_array, log_base_array, targets_array);
    validate_candidate_lambda_array(lambda_values_array, s, "lambda_values");
    validate_candidate_lambda_array(lambda_bar_array, s, "lambda_bar");
    const Config cfg = validate_config(
        max_steps, residual_tol, newton_ridge, step_cap, lambda_clip,
        line_search_steps, implicit_ridge);
    const auto* phi = static_cast<const double*>(phi_array.data());
    const auto* log_base = static_cast<const double*>(log_base_array.data());
    const auto* targets = static_cast<const double*>(targets_array.data());
    const auto* lambda_values
        = static_cast<const double*>(lambda_values_array.data());
    const auto* lambda_bar
        = static_cast<const double*>(lambda_bar_array.data());
    py::array_t<double> phi_bar_out(
        {s.candidates, s.times, s.particles, s.moments});
    py::array_t<double> log_bar_out({s.times, s.particles});
    py::array_t<double> target_bar_out({s.candidates, s.times, s.moments});
    auto* phi_bar = phi_bar_out.mutable_data();
    auto* log_bar = log_bar_out.mutable_data();
    auto* target_bar = target_bar_out.mutable_data();
    std::fill(log_bar, log_bar + log_bar_out.size(), 0.0);
    // The common base-weight cotangent is first written per candidate, then
    // reduced in ascending candidate order. This avoids OpenMP reduction-order
    // drift and makes repeated float64 results bitwise deterministic.
    std::vector<double> candidate_log_bar(
        static_cast<std::size_t>(s.candidates) * s.times * s.particles, 0.0);
    {
        py::gil_scoped_release release;
        #pragma omp parallel for schedule(static)
        for (int c = 0; c < s.candidates; ++c) {
            std::vector<double> lambda(s.moments), weights, mean, covariance;
            std::vector<double> residual, adjoint;
            for (int t = 0; t < s.times; ++t) {
                const std::size_t ct
                    = static_cast<std::size_t>(c) * s.times + t;
                const std::size_t pt
                ...
                const std::size_t wt
                    = static_cast<std::size_t>(t) * s.particles;
                const std::size_t cwt
                    = (static_cast<std::size_t>(c) * s.times + t)
                    * s.particles;
                const std::size_t mt = ct * s.moments;
                for (int j = 0; j < s.moments; ++j) {
                    lambda[j] = lambda_values[mt + j];
                }
                state(
                    phi + pt, log_base + wt, targets + mt, lambda,
                    s.particles, s.moments, weights, mean, covariance, residual);
                for (int j = 0; j < s.moments; ++j) {
                    covariance[static_cast<std::size_t>(j) * s.moments + j]
                        += cfg.implicit_ridge;
                }
                std::vector<double> rhs(s.moments);
                for (int j = 0; j < s.moments; ++j) {
                    rhs[j] = lambda_bar[mt + j];
                }
                if (!solve_dense(covariance, rhs, s.moments, adjoint)) {
                    throw std::runtime_error(
                        "singular covariance in candidate I-projection VJP");
                }
                for (int j = 0; j < s.moments; ++j) {
                    target_bar[mt + j] = adjoint[j];
                }
                for (int i = 0; i < s.particles; ++i) {
                    double centered_dot = 0.0;
                    for (int j = 0; j < s.moments; ++j) {
                        centered_dot += (
                            phi[pt + static_cast<std::size_t>(i) * s.moments + j]
                            - mean[j]) * adjoint[j];
                    }
                    candidate_log_bar[cwt + i]
                        = -weights[i] * centered_dot;
                    for (int j = 0; j < s.moments; ++j) {
                        phi_bar[pt + static_cast<std::size_t>(i) * s.moments + j]
                            = -weights[i]
                            * (adjoint[j] + lambda[j] * centered_dot);
                    }
                }
            }
        }
    ...
    for (int c = 0; c < s.candidates; ++c) {
        const std::size_t offset
            = static_cast<std::size_t>(c) * s.times * s.particles;
        for (std::size_t i = 0;
             i < static_cast<std::size_t>(s.times) * s.particles; ++i) {
            log_bar[i] += candidate_log_bar[offset + i];
        }
    }
    return py::dict(
        "phi"_a = std::move(phi_bar_out),
        "log_base_weights"_a = std::move(log_bar_out),
        "targets"_a = std::move(target_bar_out));
}

py::array_t<double> jvp_candidate_batch(
    const py::array& phi_array, const py::array& log_base_array,
    const py::array& targets_array, const py::array& lambda_values_array,
    const py::array& phi_dot_array, const py::array& log_dot_array,
    const py::array& target_dot_array, int max_steps, double residual_tol,
    double newton_ridge, double step_cap, double lambda_clip,
    int line_search_steps, double implicit_ridge) {
    const CandidateShape s = validate_candidate_inputs(
        phi_array, log_base_array, targets_array);
    const CandidateShape tangent_shape = validate_candidate_inputs(
        phi_dot_array, log_dot_array, target_dot_array);
    if (tangent_shape.candidates != s.candidates
        || tangent_shape.times != s.times
        || tangent_shape.particles != s.particles
        || tangent_shape.moments != s.moments) {
        throw py::value_error("candidate tangent shapes must match primal inputs");
    }
    validate_candidate_lambda_array(lambda_values_array, s, "lambda_values");
    const Config cfg = validate_config(
        max_steps, residual_tol, newton_ridge, step_cap, lambda_clip,
    ...
    const auto* log_dot = static_cast<const double*>(log_dot_array.data());
    const auto* target_dot = static_cast<const double*>(target_dot_array.data());
    py::array_t<double> output({s.candidates, s.times, s.moments});
    auto* lambda_dot = output.mutable_data();
    {
        py::gil_scoped_release release;
        #pragma omp parallel for schedule(static)
        for (int c = 0; c < s.candidates; ++c) {
            std::vector<double> lambda(s.moments), weights, mean, covariance;
            std::vector<double> residual, solution;
            for (int t = 0; t < s.times; ++t) {
                const std::size_t ct
                    = static_cast<std::size_t>(c) * s.times + t;
                const std::size_t pt
                ...
                std::vector<double> dF(s.moments, 0.0);
                for (int j = 0; j < s.moments; ++j) {
                    dF[j] = -target_dot[mt + j];
                }
                for (int i = 0; i < s.particles; ++i) {
                    double logit_dot = log_dot[wt + i];
                    for (int j = 0; j < s.moments; ++j) {
                        logit_dot += phi_dot[
                            pt + static_cast<std::size_t>(i) * s.moments + j]
                            * lambda[j];
                    }
                    for (int j = 0; j < s.moments; ++j) {
                        dF[j] += weights[i] * (
                ...
                if (!solve_dense(covariance, dF, s.moments, solution)) {
                    throw std::runtime_error(
                        "singular covariance in candidate I-projection JVP");
                }
                for (int j = 0; j < s.moments; ++j) {
                    lambda_dot[mt + j] = solution[j];
                }
            }
        }
    }
...
PYBIND11_MODULE(_candidate_iprojection_native, module) {
    module.doc() = "C++17/OpenMP candidate-batched empirical I-projection solver";
    ...
    module.def("jvp_batch", &jvp_batch,
               ...
               py::arg("step_cap"), py::arg("lambda_clip"), py::arg("line_search_steps"),
               py::arg("implicit_ridge"));
    module.def("solve_candidate_batch", &solve_candidate_batch,
               py::arg("phi").noconvert(), py::arg("log_base_weights").noconvert(),
               py::arg("targets").noconvert(), py::arg("max_steps"),
               py::arg("residual_tol"), py::arg("newton_ridge"), py::arg("step_cap"),
               py::arg("lambda_clip"), py::arg("line_search_steps"),
               py::arg("implicit_ridge"));
    module.def("vjp_candidate_batch", &vjp_candidate_batch,
               py::arg("phi").noconvert(), py::arg("log_base_weights").noconvert(),
               py::arg("targets").noconvert(), py::arg("lambda_values").noconvert(),
               py::arg("lambda_bar").noconvert(), py::arg("max_steps"),
               py::arg("residual_tol"), py::arg("newton_ridge"),
               py::arg("step_cap"), py::arg("lambda_clip"),
               py::arg("line_search_steps"), py::arg("implicit_ridge"));
    module.def("jvp_candidate_batch", &jvp_candidate_batch,
               py::arg("phi").noconvert(), py::arg("log_base_weights").noconvert(),
               py::arg("targets").noconvert(), py::arg("lambda_values").noconvert(),
               py::arg("phi_dot").noconvert(),
               py::arg("log_base_weights_dot").noconvert(),
               py::arg("targets_dot").noconvert(), py::arg("max_steps"),
               py::arg("residual_tol"), py::arg("newton_ridge"),
               py::arg("step_cap"), py::arg("lambda_clip"),
               py::arg("line_search_steps"), py::arg("implicit_ridge"));
}
