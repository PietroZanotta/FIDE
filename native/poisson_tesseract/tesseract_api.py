...
def apply(inputs: InputSchema) -> OutputSchema:
    ...
    result = _solve(q_operator, rhs, gauge, inputs, label="forward")
...
def vector_jacobian_product(
    ...
    q_operator, rhs, gauge = _arrays(inputs)
    psi = np.asarray(
        _solve(q_operator, rhs, gauge, inputs, label="forward-for-VJP")["psi"],
        dtype=np.float64,
        order="C",
    )
    psi_bar = np.ascontiguousarray(cotangent_vector["psi"], dtype=np.float64)
    lambda_ = np.asarray(
        _solve(q_operator, psi_bar, gauge, inputs, label="adjoint")["psi"],
        dtype=np.float64,
        order="C",
    )
...
def jacobian_vector_product(
    ...
    q_operator, rhs, gauge = _arrays(inputs)
    psi = np.asarray(
        _solve(q_operator, rhs, gauge, inputs, label="forward-for-JVP")["psi"],
        dtype=np.float64,
        order="C",
    )
