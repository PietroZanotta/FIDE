...
class ActiveNematic2D:
    ...
    def _project_force(
        ...
        pfx = fxh.copy()
        pfy = fyh.copy()
        pfx[nonzero] -= kx[nonzero] * kdotf[nonzero] / k2[nonzero]
        pfy[nonzero] -= ky[nonzero] * kdotf[nonzero] / k2[nonzero]
        pfx[~nonzero] = 0.0
        pfy[~nonzero] = 0.0
        return pfx, pfy
    ...
    def _velocity_hat_from_qhat(
        ...
        *,
        ...
        pfx, pfy = self._project_force(fxh, fyh, kx, ky, k2, nonzero)

        denom = p.viscosity * k2 + p.friction
        uh = np.zeros_like(pfx)
        vh = np.zeros_like(pfy)
        ...
        uh[nonzero] = -pfx[nonzero] / denom[nonzero]
        vh[nonzero] = -pfy[nonzero] / denom[nonzero]
        return uh, vh
    ...
    def molecular_field(self) -> Tuple[Array, Array]:
        ...
        q1p = np.fft.ifft2(q1hp).real
        q2p = np.fft.ifft2(q2hp).real
        ...
        return self._ifft_real(h1h), self._ifft_real(h2h)
    ...
    def _beris_edwards_S(
        ...
        q1p = np.fft.ifft2(self._pad_hat(q1h)).real
        q2p = np.fft.ifft2(self._pad_hat(q2h)).real

        ux = np.fft.ifft2(1j * self._kx_pad * uhp).real
        uy = np.fft.ifft2(1j * self._ky_pad * uhp).real
        vx = np.fft.ifft2(1j * self._kx_pad * vhp).real
        vy = np.fft.ifft2(1j * self._ky_pad * vhp).real
        ...
        return self._ifft_real(s1h), self._ifft_real(s2h)
    ...
    def _nonlinear_rhs_hat(
        ...
        uhp, vhp = self._velocity_hat_from_qhat(
            ...
            ky=self._ky_pad,
            k2=self._k2_pad,
            nonzero=self._nonzero_pad,
        )
        up = np.fft.ifft2(uhp).real
        vp = np.fft.ifft2(vhp).real

        q1x = np.fft.ifft2(1j * self._kx_pad * q1hp).real
        q1y = np.fft.ifft2(1j * self._ky_pad * q1hp).real
        q2x = np.fft.ifft2(1j * self._kx_pad * q2hp).real
        q2y = np.fft.ifft2(1j * self._ky_pad * q2hp).real
        adv1 = up * q1x + vp * q1y
        adv2 = up * q2x + vp * q2y
    ...
    def _advance_one(self, h: float) -> None:
        ...
        a1 = self._ifft_real(a1h)
        a2 = self._ifft_real(a2h)
        ...
        self.q1 = self._ifft_real(q1new)
        self.q2 = self._ifft_real(q2new)
        self.t += h
