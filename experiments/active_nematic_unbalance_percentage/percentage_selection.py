...
def _law_screen(exp, bank, candidates, *, risk_max: float, limit: int, mandatory):
    mandatory_keys = {
    ...
    for candidate in (first + rest)[: max(len(first), int(limit))]:
        ...
        if risk["valid"] and float(risk["value"]) <= risk_max + 1.0e-12:
...
def optimize_percentage_designs(exp, bank):
    ...
    full_screened = _law_screen(
        ...
        mandatory=[law_candidate.eta, tangent_candidate.eta],
    )
