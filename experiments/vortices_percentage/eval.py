"""Display the saved vortices validation result without recomputation."""
...
EXPECTED_SELECTION_SHA256 = "f6b45c6b7beedc66a72c8cdf05f19e94913da0555708babbabfb1cd13d6f64fc"
...
def main() -> int:
    ...
    for method in ("law", "tangent", "full"):
        ...
        table_rows.append((
            ...
            number(risk["mean"]), number(sample_sd_from_se(risk["se"], risk["n"])),
            number(tangent["mean"]), number(sample_sd_from_se(tangent["se"], tangent["n"])),
            number(full["mean"]), number(sample_sd_from_se(full["se"], full["n"])), number(full["se"]),
    ...
    print_uncertainty_note(
        "Selection has no sampling uncertainty. Validation SD is reconstructed from the saved ordinary SE and n=64 independent trials (SD = SE√n)."
    )
    return 0
