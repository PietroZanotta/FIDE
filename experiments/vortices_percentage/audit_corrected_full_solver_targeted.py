"""Targeted physical-q Full-solver audit for vortex 3% candidates."""
...
def main() -> None:
    ...
    candidates = [
        ...
        for row in load_pareto_candidates(
            pareto,
            selection_key=lambda result, method: result["selection"][
                f"{method}_optimum"
            ],
        )
        if float(row["allowance_percent"]) == 3.0
    ]
    watched = [
        pareto / "pareto.json",
    ...
    def evaluate(geometry: Any, method: str):
        return exp.evaluate_common_discretization_decomposition_exact(
            ...
            bank,
            progress_desc=f"corrected vortex 3% {method}",
        )
