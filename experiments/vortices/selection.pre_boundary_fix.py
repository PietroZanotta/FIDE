...
def _audit_population(exp, ranked, *, limit: int, min_valid: int) -> tuple[Array, list[dict[str, Any]]]:
    ...
    for _, fast, eta in ranked:
        ...
        rows.append({
            "eta": np.asarray(eta).tolist(), "fast": float(fast),
            "exact": float(exact["value"]), "valid": bool(exact["valid"]),
            "max_calibration_residual": exact.get("max_calibration_residual"),
            "min_ess_fraction": exact.get("min_ess_fraction"),
        })
    valid = [r for r in rows if r["valid"] and np.isfinite(r["exact"])]
    if not valid:
        raise RuntimeError("no exact-valid DG-Exact population candidate")
