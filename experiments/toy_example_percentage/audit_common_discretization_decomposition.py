"""Common-raster decomposition audit for saved toy Pareto candidates."""
...
def main() -> None:
    ...
    def evaluate(geometry: Any, key: str) -> list[dict[str, Any]]:
        print(f"[audit] common raster eta_deg={np.rad2deg(geometry).tolist()}", flush=True)
    ...
    summary.update(
        {
            ...
            "authoritative_evaluator": "ToyExperiment authoritative exact raster weighted-Poisson evaluator",
            "raster_grid": {"shape": [int(exp.grid.n), int(exp.grid.n)], "dx": float(exp.grid.dx)},
