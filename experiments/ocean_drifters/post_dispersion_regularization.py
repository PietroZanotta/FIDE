...
class OceanPostDispersionAction:
    ...
    def __init__(
        ...
        self.days = np.asarray(self.cfg["audit_days"], dtype=np.float64)
    ...
    def _validate(self) -> None:
        ...
        if len(self._rows) != 252:
            raise ValueError("the frozen post-dispersion audit must have 252 rows")
        if any(
        ...
        expected_days = set(self.days.tolist())
        ...
        for design in designs:
            ...
            if len(local) != len(self.days) or not np.array_equal(
                np.asarray([float(row["day"]) for row in local]), self.days
            ):
    ...
    def exact_tangent_result(self, design_index: int) -> dict[str, Any]:
        ...
        valid = bool(np.isfinite(density).all() and value >= 0.0)
        return {
            "valid": valid,
    ...
    def exact_full_result(self, design_index: int) -> dict[str, Any]:
        ...
        valid = bool(
            ...
            and value >= 0.0
            and all(_as_bool(row["direct_reference_valid"]) for row in source)
        )
        return {
            "valid": valid,
            "value": value if valid else math.inf,
    ...
    def evaluate_layout_exact(self, design_index: int) -> dict[str, Any]:
        ...
        _, regularized_ritz_density, _ = self._scaled_series(
            design, "regularized_ritz_physical_action"
        )
        _, regularized_fv_density, _ = self._scaled_series(
            design, "fv_physical_action"
        )
        regularized_ritz = float(self.time_weights @ regularized_ritz_density)
        regularized_fv = float(self.time_weights @ regularized_fv_density)
        ...
        valid = bool(
            ...
            and all(_as_bool(row["combined_floor_case_valid"]) for row in source)
        )
        return {
            "design_index": design,
            "design_id": self.design_ids[design],
