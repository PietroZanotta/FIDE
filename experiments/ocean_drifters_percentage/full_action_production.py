...
class OceanFullActionProduction:
    ...
    def __init__(self, experiment, analysis_dir: Path, output_dir: Path):
        ...
        if self.cfg.get("production_run_authorized") is not True:
            raise RuntimeError(
                "ocean production is not authorized after the relative-risk "
    ...
    def _finalize_artifacts(
        ...
        report_path.write_text(
            ...
            "## Decision\n\n"
            f"The frozen {len(summaries)}x{len(self.runner.times)} variational production sweep is "
            f"{'valid' if result['full_action_valid'] else 'not valid'}: "
            f"{result['valid_layout_count']}/{len(summaries)} layouts pass every required "
