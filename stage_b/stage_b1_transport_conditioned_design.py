#!/usr/bin/env python3
"""
Stage B.1: refined nonlinear transport-conditioned experimental design on moment fibers.
...
Recommended first run:
    python stage_b1_transport_conditioned_design.py --preset quick

Main Stage B.1 run:
    python stage_b1_transport_conditioned_design.py --preset reference

Convergence + continuous Full-TC refinement:
    python stage_b1_transport_conditioned_design.py --preset reference --optimizer --convergence
...
def main():
    ...
    parser.add_argument("--output", default="stage_b1_results.json")
    parser.add_argument("--optimizer", action="store_true", help="also run differentiable continuous Full-TC Adam refinement")
    parser.add_argument("--convergence", action="store_true", help="run frozen-design grid/time convergence at 31^2/11, 39^2/15, 51^2/21")
    parser.add_argument("--dense-angle-n", type=int, default=None, help="override angular nodes in [0,pi); reference default is 37")
    parser.add_argument("--min-sep-deg", type=float, default=None, help="override hard projective sensor separation; reference default is 20 degrees")
    parser.add_argument("--tau", type=float, default=None, help="override primary relative Lift tolerance; default 0.05")
    ...
    t0 = time.time()
    print("Stage B.1: refined nonlinear transport-conditioned design on moment fibers")
    print(
        f"preset={args.preset}, grid={cfg.grid_n}x{cfg.grid_n}, time_n={cfg.time_n}, "
    ...
    convergence = None
    if args.convergence:
        convergence = fixed_design_convergence_study(model, selected)
    ...
    result = {
        "stage": "B.1",
        "preset": args.preset,
        "run_convergence": bool(args.convergence),
        "purpose": "refined nonlinear moment-fiber design: enforce nonredundant sensors, refine the design oracle, compare local tangent conditioning with full weighted-Poisson law-path conditioning under matched Lift constraints, and optionally check discretization convergence",
        "software": {
    ...
    print_key_results(model, selected, rows, grad_checks, sweep, opt, convergence)
