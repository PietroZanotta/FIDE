#!/usr/bin/env python3
"""
Stage B.3: high-resolution convergence confirmation for transport-conditioned
experimental design on nonlinear moment fibers.

This is deliberately NOT a new scientific benchmark. It loads the exact Stage B.2
configuration and selected designs from stage_b2_results_reference.json, then:

  1. re-evaluates the frozen B.2 Lift, Tangent-TC, and Full-TC designs at the
     B.2 fine resolution (normally 51x51, time_n=21) and at a higher resolution
     (default 65x65, time_n=27);
  2. reports fixed-design discretization changes in Lift, full action, tangent
     action, and hidden action;
  3. performs only three local/continuous high-resolution SLSQP refinements
     (Lift, Tangent-TC, Full-TC), initialized from the already identified B.2
     basins -- there is NO new dense global scan;
  4. recomputes the matched-Lift bound at high resolution and reports the final
     Full-TC vs Lift and Full-TC vs Tangent-TC full-action reductions;
  5. saves all results to one JSON file.

Recommended run:
    python stage_b3_resolution_confirmation.py \
        --b2-json stage_b2_results_reference.json \
        --high-grid-n 65 --high-time-n 27 \
        --output stage_b3_results_reference.json

The purpose is numerical confirmation only. Scientific parameters, sensor family,
scenario prior, separation constraint, and Lift tolerance are inherited unchanged
from Stage B.2.
"""
...
# -----------------------------------------------------------------------------
# Stage B.3: high-resolution confirmation only
# -----------------------------------------------------------------------------

def _load_b2_state(path: Path) -> Tuple[Config, Dict[str, np.ndarray], Dict[str, Any]]:
    data = json.loads(path.read_text(encoding="utf-8"))
    if "base_config" not in data or "fine_continuous" not in data:
        raise ValueError(
            "B.2 JSON must contain 'base_config' and 'fine_continuous'. "
            "Please pass the JSON produced by stage_b2_transport_conditioned_design.py."
        )

    raw_cfg = data["base_config"]
    valid = {f.name for f in dataclasses.fields(Config)}
    cfg_kwargs = {k: raw_cfg[k] for k in raw_cfg if k in valid}
    cfg = Config(**cfg_kwargs)

    fine = data["fine_continuous"]
    designs = {}
    for key, label in (("lift", "lift"), ("tangent", "tangent_tc"), ("full", "full_tc")):
        b = fine[key]["best"]
        designs[label] = canonical_eta_np(
            np.array([float(b["theta1_rad"]), float(b["theta2_rad"])], dtype=np.float64)
        )
    return cfg, designs, data


def _local_start_cloud(
    centers: List[np.ndarray],
    min_sep_deg: float,
    radius_deg: float,
    points: int,
    ...
    starts: List[np.ndarray] = []
    for c in centers:
        starts.append(canonical_eta_np(c))
        for da in offsets:
            for db in offsets:
                starts.append(canonical_eta_np(c + np.array([da, db], dtype=np.float64)))
    # Rank-independent de-duplication; SLSQP itself decides the basin quality.
    return _unique_starts(starts, min_sep_deg=min_sep_deg, max_count=max_count)


def _fixed_design_evaluation(
    cfg: Config,
    designs: Dict[str, np.ndarray],
    resolutions: List[Tuple[int, int]],
) -> Dict[str, Any]:
    rows: List[Dict[str, Any]] = []
    model_cache: Dict[Tuple[int, int], StageB] = {}
    print("\nFrozen-design high-resolution convergence")
    print("  scientific model and sensor angles are frozen; only grid/time resolution changes")
    for grid_n, time_n in resolutions:
        model = make_fine_model(cfg, grid_n, time_n)
        model_cache[(grid_n, time_n)] = model
        drows = {label: design_row(model, eta) for label, eta in designs.items()}
        rows.append({"grid_n": int(grid_n), "time_n": int(time_n), "designs": drows})
        print(f"  completed grid={grid_n}x{grid_n}, time_n={time_n}", flush=True)

    ref = rows[-1]
    rel: Dict[str, Any] = {}
    for label in designs:
        rr = ref["designs"][label]
        entries = []
        for row in rows:
            cur = row["designs"][label]
            item = {"grid_n": row["grid_n"], "time_n": row["time_n"]}
            for metric in ("lift_mmd2", "full_action", "tangent_action", "hidden_action"):
                item[f"{metric}_relative_error_vs_high"] = abs(cur[metric] - rr[metric]) / max(abs(rr[metric]), 1e-12)
            entries.append(item)
        rel[label] = entries

    return {
        "resolutions": rows,
        "relative_errors_vs_high": rel,
        "high_resolution": {"grid_n": ref["grid_n"], "time_n": ref["time_n"]},
        "model_cache": model_cache,  # removed before JSON serialization
    }


def _print_b3_report(
    cfg: Config,
    frozen: Dict[str, Any],
    ...
    print("\n" + "=" * 88)
    print("STAGE B.3 — HIGH-RESOLUTION CONFIRMATION")
    print("=" * 88)
    print(f"Inherited Lift tolerance          : {100*cfg.lift_tau:.2f}%")
    print(f"Inherited min sensor separation   : {cfg.min_sep_deg:.2f}°")

    print("\nFrozen B.2 design convergence")
    high = frozen["resolutions"][-1]
    low = frozen["resolutions"][-2]
    for label, pretty in (("lift", "Lift"), ("tangent_tc", "Tangent-TC"), ("full_tc", "Full-TC")):
        h = high["designs"][label]
        l = low["designs"][label]
        r = frozen["relative_errors_vs_high"][label][-2]
        print(
            f"  {pretty:10s}: ({h['theta1_deg']:.2f}°, {h['theta2_deg']:.2f}°) | "
            f"high Full={h['full_action']:.8e} | high Tangent={h['tangent_action']:.8e} | "
            f"51→65 ΔFull={100*r['full_action_relative_error_vs_high']:.3f}% | "
            f"ΔTangent={100*r['tangent_action_relative_error_vs_high']:.3f}%"
        )

    lift = refined["lift"]["best"]
    tang = refined["tangent"]["best"]
    full = refined["full"]["best"]
    lift_star = refined["lift_star"]
    lift_max = refined["lift_max"]
    headroom = 1.0 - full["full_action"] / lift["full_action"]
    vs_tangent = 1.0 - full["full_action"] / tang["full_action"]
    sacrifice = full["lift_mmd2"] / lift_star - 1.0
    basin = math.degrees(unordered_design_distance(
    ...
    print("\nHIGH-RESOLUTION REFINED FINAL COMPARISON")
    print(f"  resolution                     : {refined['grid_n']}x{refined['grid_n']}, time_n={refined['time_n']}")
    print(f"  high-res Lift*                 : {lift_star:.8e}")
    print(f"  high-res Lift bound            : {lift_max:.8e}")
    print(f"  Lift optimum                   : ({lift['theta1_deg']:.2f}°, {lift['theta2_deg']:.2f}°)")
    print(f"  Tangent-TC optimum             : ({tang['theta1_deg']:.2f}°, {tang['theta2_deg']:.2f}°)")
    print(f"  Full-TC optimum                : ({full['theta1_deg']:.2f}°, {full['theta2_deg']:.2f}°)")
    print(f"  Full-TC Lift sacrifice         : {100*sacrifice:+.3f}%")
    ...
    print("\nHigh-resolution SLSQP status")
    for key, pretty in (("lift", "Lift"), ("tangent", "Tangent-TC"), ("full", "Full-TC")):
        b = refined[key]["best"]
        print(
            f"  {pretty:10s}: success={b['scipy_success']} feasible={b['feasible']} "
            f"nit={b['nit']} message={b['scipy_message']}"
        )

    print("\nDeclared confirmation checks")
...
def main() -> None:
    ...
    parser.add_argument(
        "--b2-json", type=Path, default=Path("stage_b2_results_reference.json"),
        help="JSON produced by Stage B.2; exact scientific config and final B.2 designs are loaded from it.",
    )
    parser.add_argument("--high-grid-n", type=int, default=65)
    parser.add_argument("--high-time-n", type=int, default=27)
    parser.add_argument(
        "--refine-radius-deg", type=float, default=2.5,
        help="small start-cloud radius around the already identified B.2 basins",
    )
    parser.add_argument("--refine-points", type=int, default=3)
    parser.add_argument("--slsqp-starts", type=int, default=10)
    parser.add_argument("--slsqp-maxiter", type=int, default=70)
    parser.add_argument(
        "--full-action-conv-tol", type=float, default=0.02,
        help="required 51->high relative full-action change for Tangent-TC and Full-TC",
    )
    parser.add_argument(
        "--lift-full-action-conv-tol", type=float, default=0.05,
        help="required 51->high relative full-action change for the Lift baseline",
    )
    parser.add_argument(
        "--tangent-action-conv-tol", type=float, default=0.05,
        help="required 51->high relative tangent-action change for the Tangent-TC frozen design",
    )
    parser.add_argument("--output", type=Path, default=Path("stage_b3_results_reference.json"))
    args = parser.parse_args()

    if not args.b2_json.exists():
        raise FileNotFoundError(f"Stage B.2 JSON not found: {args.b2_json}")
    if args.high_grid_n < 51 or args.high_time_n < 21:
        raise ValueError("B.3 high resolution should be at least 51x51 / time_n=21")

    t0 = time.time()
    cfg, frozen_designs, b2_data = _load_b2_state(args.b2_json)
    b2_fine = b2_data["fine_continuous"]
    base_fine_grid = int(b2_fine.get("grid_n", 51))
    base_fine_time = int(b2_fine.get("time_n", 21))

    print("Stage B.3: high-resolution confirmation only")
    print(
        f"B.2 fine={base_fine_grid}x{base_fine_grid}/time{base_fine_time}, "
        f"high={args.high_grid_n}x{args.high_grid_n}/time{args.high_time_n}, "
        f"alpha_n={cfg.alpha_n}, min_sep={cfg.min_sep_deg:.1f}°, tau={100*cfg.lift_tau:.1f}%"
    )
    print(f"JAX backend={jax.default_backend()}, devices={jax.devices()}, x64={jax.config.jax_enable_x64}")

    # 1) Frozen B.2 designs at 51/21 and 65/27 (or user-selected high resolution).
    frozen = _fixed_design_evaluation(
        cfg,
        frozen_designs,
        resolutions=[(base_fine_grid, base_fine_time), (args.high_grid_n, args.high_time_n)],
    )
    high_model = frozen["model_cache"][(args.high_grid_n, args.high_time_n)]

    # 2) Small high-resolution continuous refinements from the already identified basins.
    print("\nRunning high-resolution local/continuous SLSQP refinements ...")
    centers = [frozen_designs["lift"], frozen_designs["tangent_tc"], frozen_designs["full_tc"]]
    starts = _local_start_cloud(
        centers,
        min_sep_deg=cfg.min_sep_deg,
        radius_deg=args.refine_radius_deg,
        points=args.refine_points,
        max_count=args.slsqp_starts,
    )

    high_lift = constrained_slsqp(
        high_model, "lift", starts, None,
        maxiter=args.slsqp_maxiter,
    )
    high_lift_star = float(high_lift["best"]["lift_mmd2"])
    high_lift_max = float((1.0 + cfg.lift_tau) * high_lift_star)

    # Seed tangent/full with the high-res Lift optimum too, while remaining local to known basins.
    constrained_centers = centers + [np.array([
        high_lift["best"]["theta1_rad"], high_lift["best"]["theta2_rad"]
    ], dtype=np.float64)]
    constrained_starts = _local_start_cloud(
        constrained_centers,
        min_sep_deg=cfg.min_sep_deg,
        radius_deg=args.refine_radius_deg,
        points=args.refine_points,
    ...
    high_tangent = constrained_slsqp(
        high_model, "tangent", constrained_starts, high_lift_max,
        maxiter=args.slsqp_maxiter,
    )
    high_full = constrained_slsqp(
        high_model, "full", constrained_starts, high_lift_max,
        maxiter=args.slsqp_maxiter,
    )

    refined = {
        "grid_n": int(args.high_grid_n),
        "time_n": int(args.high_time_n),
        "lift_star": high_lift_star,
        "lift_max": high_lift_max,
        "lift": high_lift,
        "tangent": high_tangent,
        "full": high_full,
    }

    # 3) Numerical confirmation checks.
    rel = frozen["relative_errors_vs_high"]
    lift_full_change = rel["lift"][-2]["full_action_relative_error_vs_high"]
    tang_full_change = rel["tangent_tc"][-2]["full_action_relative_error_vs_high"]
    full_full_change = rel["full_tc"][-2]["full_action_relative_error_vs_high"]
    tang_tangent_change = rel["tangent_tc"][-2]["tangent_action_relative_error_vs_high"]

    hb_lift = high_lift["best"]
    hb_tang = high_tangent["best"]
    hb_full = high_full["best"]
    checks = {
        "frozen_full_tc_full_action_converged": bool(full_full_change <= args.full_action_conv_tol),
        "frozen_tangent_tc_full_action_converged": bool(tang_full_change <= args.full_action_conv_tol),
        "frozen_lift_full_action_converged": bool(lift_full_change <= args.lift_full_action_conv_tol),
        "frozen_tangent_tc_tangent_action_converged": bool(tang_tangent_change <= args.tangent_action_conv_tol),
        "high_lift_slsqp_success_feasible": bool(hb_lift["scipy_success"] and hb_lift["feasible"]),
        "high_tangent_slsqp_success_feasible": bool(hb_tang["scipy_success"] and hb_tang["feasible"]),
        "high_full_slsqp_success_feasible": bool(hb_full["scipy_success"] and hb_full["feasible"]),
        "high_full_within_lift_bound": bool(hb_full["lift_mmd2"] <= high_lift_max * (1.0 + 2e-6)),
        "high_full_sensor_separation": bool(hb_full["sensor_separation_deg"] >= cfg.min_sep_deg - 2e-5),
        "high_full_action_below_lift": bool(hb_full["full_action"] < hb_lift["full_action"]),
        "high_full_action_below_tangent_tc": bool(hb_full["full_action"] < hb_tang["full_action"]),
    }

    # Derived scientific comparisons.
    headroom = 1.0 - hb_full["full_action"] / hb_lift["full_action"]
    vs_tangent = 1.0 - hb_full["full_action"] / hb_tang["full_action"]
    sacrifice = hb_full["lift_mmd2"] / high_lift_star - 1.0
    basin = math.degrees(unordered_design_distance(
        np.array([hb_full["theta1_rad"], hb_full["theta2_rad"]]),
        np.array([hb_tang["theta1_rad"], hb_tang["theta2_rad"]]),
    ))

    # Remove non-serializable models.
    frozen_json = dict(frozen)
    frozen_json.pop("model_cache", None)

    result = {
        "stage": "B.3",
        "purpose": "high-resolution numerical confirmation of the frozen Stage B.2 scientific benchmark",
        "source_b2_json": str(args.b2_json),
        "software": {
        ...
        "inherited_config": jsonify(cfg),
        "b3_config": {
            "base_fine_grid_n": base_fine_grid,
            "base_fine_time_n": base_fine_time,
            "high_grid_n": args.high_grid_n,
            "high_time_n": args.high_time_n,
            "refine_radius_deg": args.refine_radius_deg,
            "refine_points": args.refine_points,
            "slsqp_starts": args.slsqp_starts,
            "slsqp_maxiter": args.slsqp_maxiter,
            "full_action_conv_tol": args.full_action_conv_tol,
            "lift_full_action_conv_tol": args.lift_full_action_conv_tol,
            "tangent_action_conv_tol": args.tangent_action_conv_tol,
        },
        "frozen_b2_designs_rad": {k: v.tolist() for k, v in frozen_designs.items()},
        "frozen_convergence": frozen_json,
        "high_resolution_refined": refined,
        "derived": {
            "transport_headroom_fraction_vs_lift": float(headroom),
            "full_tc_vs_tangent_tc_full_action_reduction_fraction": float(vs_tangent),
            "full_tc_lift_sacrifice_fraction": float(sacrifice),
            "full_vs_tangent_design_distance_deg": float(basin),
            "frozen_51_to_high_relative_changes": {
                "lift_full_action": float(lift_full_change),
                "tangent_tc_full_action": float(tang_full_change),
                "full_tc_full_action": float(full_full_change),
                "tangent_tc_tangent_action": float(tang_tangent_change),
            },
        },
        "checks": checks,
    ...
    _print_b3_report(cfg, frozen_json, refined, checks)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(jsonify(result), indent=2) + "\n", encoding="utf-8")
